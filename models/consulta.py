...
class Consulta:
    ...
    @classmethod
    def check(cls,data):
        query = "SELECT * FROM consultas WHERE id_doctor=%(id_doctor)s and fecha_hora=%(fecha_hora)s;" #id_paciente=%(id_paciente)s and
    ...
    def agendar(cls,data):
        query = "insert into consultas (id_paciente, id_doctor, fecha_hora, comentario, pago_consulta, pago_tratamiento, created_at, updated_at) \
            values (%(id_paciente)s, %(id_doctor)s, %(fecha_hora)s, %(comentario)s, 150000, 750000, NOW() ,NOW());"
        print("Final agendar\n")
